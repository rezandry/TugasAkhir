...
__date__    = '2016-July-3'   
__url__     = 'https://engineering.purdue.edu/kak/distWatershed/Watershed-2.0.4.html'
...
CONSTRUCTOR PARAMETERS: 
    ...
    color_filter: The color filter can be specified in one of two different
                   ways: as a triple of scalars or as a triple of pairs. If
                   you specify the filter as a triple of three scalar
                   values, each scalar is used to multiply the
...
Starting with Version 2.0.2, you can apply the segmentation algorithm to
just a portion of the input image.  You can specify the image portion
through two different ways: (1) You can create a rectangular portion by
clicking your mouse pointer at the upper left corner of the rectangular
area you are interested in and then dragging the mouse pointer to its lower
right corner. And (2) You can specify any polygonal shape by simply
clicking clockwise at the points corresponding to the vertices of the
polygon.  The method to call for the first way of defining an area for
segmentation is:
...
Starting with Version 2.0.2, you can now also apply a color filter to an
input image before it is segmented.  A color is specified by the
constructor option "color_filter" and the methods to call in your script
for applying the filter are:

        apply_color_filter_hsv()

        apply_color_filter_rgb()

The following script in the Examples directory illustrates segmenting just
a portion on an image and using a color filter:
...
#        self.displayImage6(Watershed.extracted_image, "Extracted_Image -- close window when done viewing")
...
#        self.displayImage6(Watershed.extracted_image, "Extracted_Image -- close window when done viewing")
    ...
    def apply_otsu_threshold_to_hue(self, polarity=1, area_threshold=0):
        '''
...
#        hsv_new_image = hsv_new_image.resize((int(width/4),int(height/4)), Image.ANTIALIAS)
        ...
        for i in range(0, mask_size[0]):
            for j in range(0, mask_size[1]):
                if hsv_new_image.getpixel((j,i))[2] > 50:
                    mask_array[(i,j)] = 1
        if self.debug:
            for i in range(0, height):
                sys.stdout.write("\n")
                for j in range(0, width):
                    if mask_array[(i,j)] == 1:
                        ...
                        sys.stdout.write('0')
            sys.stdout.write('\n\n')        
        return self._connected_components_for_binary_array(mask_array, area_threshold)
...
#        self.displayImage3(original_image, "Watershed Segmentation (close window when done viewing)")
    ...
    def displayImage(self, argimage, title=""):
        ...
        tk.after(1000, self._callbak, tk)    # display will stay on for just one second
        tk.mainloop()
    ...
    def displayImage4(self, argimage, title=""):
        '''
        Displays the argument image in its actual size without imposing the constraint
        that the larger dimension of the image be at most half the corresponding screen
        dimension.
        '''
        width,height = argimage.size
        tk = Tkinter.Tk()
...
#        self.displayImage3(output_image, "RESULT:  a segment of the image")
    ...
    def _connected_components_for_binary_array(self, array, area_threshold): 
        ...
        binary blobs.  And the second parameter, 'area_threshold', is used to reject the blobs 
        whose area is less than this threshold.  The method returns the total number of blobs 
        ...
        if self.debug:
            print("\nblob sizes: %s" % str(blob_sizes))
        num_large_components = len([x for x in self.blob_dictionary if len(self.blob_dictionary[x]) > area_threshold])
